...
export const houseService = {
  async createHouse(houseData: CreateHouseRequest): Promise<House> {
    const response = await api.post<House>(API_ENDPOINTS.POST_CREATE_HOUSE, houseData);
    
    // Store the new house as current house
    await AsyncStorage.setItem(STORAGE_KEYS.CURRENT_HOUSE, JSON.stringify(response.data));
    
    return response.data;
  },

  async joinHouse(joinData: JoinHouseRequest): Promise<House> {
    const response = await api.post<House>(API_ENDPOINTS.POST_JOIN_HOUSE, joinData);
  ...
  async getUserHouses(): Promise<House[]> {
    ...
    await AsyncStorage.setItem(STORAGE_KEYS.HOUSES, JSON.stringify(response.data));
    
    return response.data;
  },

  async getHouseDetails(houseId: string): Promise<House> {
    const response = await api.get<House>(API_ENDPOINTS.GET_HOUSE_BY_ID(houseId));
    return response.data;
  },

  async updateHouse(houseId: string, updateData: UpdateHouseRequest): Promise<House> {
    const response = await api.patch<House>(API_ENDPOINTS.PATCH_HOUSE_BY_ID(houseId), updateData);
  ...
  async uploadHouseImage(houseId: string, imageUri: string): Promise<House> {
    ...
    const response = await api.post<House>(API_ENDPOINTS.POST_UPLOAD_HOUSE_IMAGE(houseId), formData, {
      headers: {
  ...
  async setCurrentHouse(house: House): Promise<void> {
    await AsyncStorage.setItem(STORAGE_KEYS.CURRENT_HOUSE, JSON.stringify(house));
  },
  ...
  validateDisplayName(displayName: string, minLength: number = 2, maxLength: number = 30): boolean {
    const trimmed = displayName.trim();
    return trimmed.length >= minLength && trimmed.length <= maxLength;
  },

  // Utility method to validate house names
  validateHouseName(houseName: string, minLength: number = 2, maxLength: number = 50): boolean {
